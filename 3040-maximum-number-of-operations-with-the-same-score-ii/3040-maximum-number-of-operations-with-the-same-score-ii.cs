public class Solution {
    ...
    public int Solve(int[] nums, int st, int en, int score, int[,] dp)
    {
       ...
       if (score == -1 || score == nums[st] + nums[st + 1])
           maxOperation = dp[st, en] = Solve(nums, st + 2, en, nums[st] + nums[st + 1], dp) + 1;
       if (score == -1 || score == nums[en] + nums[en - 1])
           maxOperation = dp[st, en] = Math.Max(maxOperation, Solve(nums, st, en - 2, nums[en] + nums[en - 1], dp) + 1);
       if (score == -1 || score == nums[st] + nums[en])
           maxOperation = dp[st, en] = Math.Max(maxOperation, Solve(nums, st + 1, en - 1, nums[st] + nums[en], dp) + 1);
