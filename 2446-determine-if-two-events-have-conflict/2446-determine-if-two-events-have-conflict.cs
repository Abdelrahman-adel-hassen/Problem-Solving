...
public  bool HaveConflict(string[] event1, string[] event2)
{
    ...
    return IsIntersect(event1StartTime, event2StartTime, event2EndTime) || IsIntersect(event2StartTime, event1StartTime, event1EndTime);
}
private bool IsIntersect(int event1StartTime, int event2StartTime, int event2EndTime)
{
    return event1StartTime <= event2EndTime && event1StartTime >= event2StartTime;
}
}